...
UserSchema.methods.addLikes = function(slug, callback){
  var user=this;
  user.likes.push(slug);
  return user.save(callback);
};

// TODO should better test for removeLikes
UserSchema.methods.removeLikes = function(slug, callback){
  var idx=this.likes.indexOf((slug))
  if(idx===-1){
...
//
// like product
UserSchema.statics.like=function(id,slug,callback){
  var Users=this.model('Users');
  ...
  return Users.findOne({id:id}).exec(function (err, user) {
    ...
    // remove like?
    if (user.likes&&user.likes.indexOf(slug)!==-1){
        return user.removeLikes(slug,callback)
    }
    return user.addLikes(slug,callback)
  });
};
...
UserSchema.statics.updateStatus=function(id, status,callback){
  ...
  return Users.findOne(id).populate('shops').exec(function (err, user) {
    ...
    user.save(function (err) {
      ...
      //
      // update all status
      bus.emit('user.update.status', user,function (err) {
      });
